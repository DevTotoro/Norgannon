...
class TSIDFormatterFactory : AnnotationFormatterFactory<TSIDParam> {
    ...
    override fun getPrinter(annotation: TSIDParam, fieldType: Class<*>): Printer<Long> {
        return Printer { objectValue, _ -> TSID.from(objectValue).toString() }
    }

    override fun getParser(annotation: TSIDParam, fieldType: Class<*>): Parser<Long> {
        return Parser { text, _ ->
